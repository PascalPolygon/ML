...
if __name__ == '__main__':
    ...
    if attrs[keyList[0]][0] == 'continuous':
        ...
        percentCorrupt = 0.02
        while percentCorrupt < 0.2:
            trainingExamples = load_examples(IRIS_TRAIN_FILE)
            testExamples = load_examples(IRIS_TEST_FILE)
            attrs, target = load_attributes(IRIS_ATTR_FILE)
            # print(dataUtils.get_labels(trainingExamples))
            trainingExamples = dataUtils.corrupt_data(trainingExamples, uTargets.copy(), percentCorrupt)
            # print(dataUtils.get_labels(trainingExamples))
            print('-----------------------------------------------')
            print(f'__main__ Corruption percent - {percentCorrupt*100}%')
            print('-----------------------------------------------')
            percentCorrupt += 0.02
